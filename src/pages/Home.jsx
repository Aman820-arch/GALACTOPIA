...
import SliceableWrapper from '../components/SliceableWrapper';
    ...
    <div className="space-y-16 animate-in fade-in duration-500 flex flex-col min-h-[80vh] justify-between">
      <div className="space-y-16">
        <header className="max-w-2xl space-y-4">
          <h1 className="text-sm font-black tracking-[0.4em] text-amber-500 uppercase animate-pulse">GALACTOPIA</h1>
        ...
        <section className="space-y-6">
          <div className="w-full overflow-hidden border-y border-white/[0.02] bg-gradient-to-r from-transparent via-white/[0.01] to-transparent py-8 relative">
            {movies.length > 0 ? (
              <div className="flex flex-row flex-nowrap w-max gap-6 animate-marquee-loop hover:[animation-play-state:paused]">
                {movies.map((movie, idx) => (
                  <div key={`${movie.id}-${idx}`} className="flex-shrink-0">
                    
                    {/* Slicing wrapper catches high velocity swipes inside marquee channel */}
                    <SliceableWrapper>
                      <div 
                        onClick={() => setSelectedMovie(movie)}
                        className="w-64 h-44 rounded-2xl border border-white/[0.04] bg-[#0c0c12]/40 p-5 flex flex-col justify-between group hover:border-amber-500/30 transition-all duration-500 cursor-pointer relative overflow-hidden"
                      >
                        {movie.poster ? (
                          <div 
      ...
      {/* Futuristic, Reactively Glowing Footer Area */}
      <footer className="mt-20 pt-8 border-t border-white/[0.03] space-y-6 group/footer">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="text-right font-mono text-[9px] tracking-widest text-zinc-600 uppercase group-hover/footer:text-zinc-400 transition-colors duration-500">
            ARCHIVE SUBSYSTEM v2.6 // DEVELOPED BY <span className="text-zinc-500 group-hover/footer:text-amber-500 transition-colors duration-500 font-bold">AMAN</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-[9px] font-mono text-zinc-600 tracking-wider">
          ...
          <a 
            ...
            className="p-2 rounded-lg bg-white/[0.02] border border-white/[0.05] text-zinc-400 hover:text-white hover:border-amber-500/20 hover:bg-amber-500/5 transition-all shadow-sm"
          >
            <svg className="w-3.5 h-3.5 transition-transform group-hover/footer:rotate-12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path></svg>
          </a>
        </div>
      </footer>
    </div>
